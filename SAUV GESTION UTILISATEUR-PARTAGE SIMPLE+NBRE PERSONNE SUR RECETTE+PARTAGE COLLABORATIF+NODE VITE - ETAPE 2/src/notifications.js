...
async function acceptCollaborativePlan(shareId, planId) {
    const currentUserId = getCurrentUserId();
    if (!currentUserId || !planId) return;

    try {
        const planRef = doc(db, 'plans', planId);
        await updateDoc(planRef, {
            collaborators: arrayUnion(currentUserId)
        });
        ...
        console.error("Erreur lors de l'acceptation de l'invitation collaborative : ", error);
        alert("Une erreur est survenue.");
    }
}
...
function renderNotification(notification) {
    ...
    if (notification.data.type === 'collaborative_plan_invite') {
        ...
        const acceptBtn = createButton('Accepter', 'btn-secondary', (e) => {
            ...
            acceptCollaborativePlan(notification.id, notification.data.planId);
        });
