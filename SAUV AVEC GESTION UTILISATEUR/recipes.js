...
import { collection, getDocs, doc, setDoc, addDoc, deleteDoc } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
export default function init() {
    ...
    // --- Style Maps ---
    const categoryBgColorMap = { 'PLAT': 'bg-blue-100', 'DESSERT': 'bg-purple-100', 'ENTREE': 'bg-green-100', 'ACCOMPAGNEMENT': 'bg-yellow-100', 'PETIT-DEJEUNER': 'bg-orange-100', 'BOISSON': 'bg-cyan-100', 'SAUCE': 'bg-red-100' };
    const categoryTextColorMap = { 'PLAT': 'text-blue-800', 'DESSERT': 'text-purple-800', 'ENTREE': 'text-green-800', 'ACCOMPAGNEMENT': 'text-yellow-800', 'PETIT-DEJEUNER': 'text-orange-800', 'BOISSON': 'text-cyan-800', 'SAUCE': 'text-red-800' };
    ...
    function renderRecipes() {
        console.log("renderRecipes called. allRecipes:", allRecipes, "activeCategory:", activeCategory, "searchTerm:", searchTerm);
        if (!recipeListContainer) return;
        recipeListContainer.innerHTML = '';
        let filteredRecipes = allRecipes;
        if (searchTerm) {
            const lowerCaseSearchTerm = searchTerm.toLowerCase();
            filteredRecipes = allRecipes.filter(recipe => recipe.name.toLowerCase().includes(lowerCaseSearchTerm));
        }
        const recipesForCategory = filteredRecipes.filter(r => (r.category || 'Non classé') === activeCategory);
    ...
    function createRecipeCard(recipe) {
        ...
        deleteButton.className = 'btn btn-ghost text-red-600 hover:bg-red-200 btn-xs';
