...
export default function init() {
    ...
    function createVerticalServingsControl(currentValue, isOverridden, changeCallback) {
        const container = document.createElement('div');
    ...
    async function loadPlanForWeek(week) {
        ...
        console.log(`Recherche de plans partagés pour userId: ${userId}, week: ${week}`);
        const q = query(plansRef, where("userId", "==", userId), where("isShared", "==", true), where("week", "==", week));
        const sharedPlansSnap = await getDocs(q);
        console.log(`Trouvé ${sharedPlansSnap.size} plan(s) partagé(s).`);
        sharedPlansSnap.forEach(doc => {
            console.log("Plan partagé trouvé :", doc.data());
            availablePlans.push({ id: doc.id, ...doc.data() });
        });

        populatePlanSelector();
        loadPlan(availablePlans[0]); // Load the user's own plan by default
    }
    ...
    function loadPlan(plan) {
        ...
        if (elements.defaultServingsControl) {
            ...
            const defaultServingsComponent = createServingsControl(defaultNumPeople, async (newValue) => {
                ...
                renderPlanner(); // Re-render to update placeholders
        ...
        const isShared = plan.isShared || false;
        elements.sharePlanBtn.disabled = isShared;
        elements.clearMenuBtn.disabled = isShared;
    ...
    async function saveCurrentPlan() {
        ...
        const planToSave = { menuData, servingsData, remarksData, defaultNumPeople: defaultNumPeople, startDay: startDay, lastUpdated: new Date() };
        try {
    ...
    function renderPlanner() {
        ...
        weekDays.forEach(dayName => {
            ...
            mealTypes.forEach(mealType => {
                // Add servings control cell
                const servingsCell = document.createElement('div');
                const servingsKey = `${dayOriginalIndex}-${mealType}`;
                const currentValue = servingsData[servingsKey] || defaultNumPeople;
                const isOverridden = servingsData.hasOwnProperty(servingsKey);
                ...
                if (isOverridden) {
                    cellClasses += ' bg-tomato'; // Highlight for overridden values
                } else {
                    cellClasses += (mealType === 'lunch') ? ' bg-amber-50' : ' bg-stone-100';
                }
    ...
    function setupEventListeners() {
        ...
        elements.defaultServingsInput?.addEventListener('change', async (event) => {
            ...
            renderPlanner(); // Re-render to update placeholders in servings inputs
            await saveCurrentPlan();
            await generateShoppingListFromPlan();
        });
    ...
    async function handleServingsChange(servingsKey, newValue) {
        ...
        renderPlanner(); // Re-render to apply style changes
        await saveCurrentPlan();
        await generateShoppingListFromPlan();
    }
