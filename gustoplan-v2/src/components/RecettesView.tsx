...
import { subscribeRecettes, deleteRecette, toggleFavoriRecette, saveRecette } from "../services/db";
import { useAuth } from "../contexts/AuthContext";
import { Recette } from "../types";
import { Plus, Trash2, Heart, Search, BookOpen, X, Edit3 } from "lucide-react";
import { RecipeEditModal } from "./RecipeEditModal";

export const RecettesView: React.FC = () => {
  const { foyer, showToast } = useAuth();
  ...
  // Formulaire d'ajout/édition via le composant réutilisable
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingRecipe, setEditingRecipe] = useState<Recette | null>(null);

  // Récupération en temps réel des recettes
  useEffect(() => {
    if (!foyer?.id) return;
    const unsubscribe = subscribeRecettes(foyer.id, setRecettes);
    return () => {
      unsubscribe();
    };
  }, [foyer?.id]);
  ...
  const handleCreateRecipe = (cat: 'entree' | 'plat' | 'dessert' | 'accompagnement') => {
    setEditingRecipe({
      id: "",
      titre: "",
      portionsDefaut: 4,
    ...
    <div className="h-full flex flex-col p-4 md:p-6 bg-slate-50 text-slate-800 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            onClick={() => handleCreateRecipe("accompagnement")}
          ...
          <button
            onClick={() => handleCreateRecipe("plat")}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-white border border-slate-200 rounded-xl pl-11 pr-10 py-3 placeholder-slate-400 text-slate-855 focus:outline-none focus:border-indigo-400 focus:bg-white transition-all shadow-2xs text-sm"
          />
          {search && (
            <button
              type="button"
