...
import { Loader2, ShoppingCart, Trash2, Plus, Minus, ChevronDown, ChevronRight, Package, Carrot, Beef, Fish, Milk, Croissant as Bread, Utensils } from "lucide-react"
                                        ...
                                        <div className="space-y-1 pl-2">
                                            {groupedList[cat].map(item => (
                                                <div key={`${item.name}-${item.unit}`} className={cn(
                                                    "group grid grid-cols-[1fr_auto_auto] gap-2 items-center p-2 rounded-lg text-xs transition-all hover:bg-muted/50 border border-transparent hover:border-border",
                                                    item.isChecked ? "opacity-50" : ""
                                                )}>
                                                    {/* Col 1: Checkbox + Image + Name */}
                                                    <div className="flex items-center gap-2 min-w-0">
                                                        ...
                                                        <span className={cn("font-medium truncate", item.isChecked && "line-through decoration-muted-foreground")}>
                                                            {item.name}
                                                        </span>
                                                    </div>

                                                    {/* Col 2: Quantity */}
                                                    <div className="text-xs font-black text-right whitespace-nowrap">
                                                        {item.totalQuantity % 1 === 0 ? item.totalQuantity : item.totalQuantity.toFixed(2)} <span className="text-[10px] font-normal text-muted-foreground">{item.unit === 'pièce(s)' ? 'pce' : item.unit}</span>
                                                    </div>
                                                    ...
                                                    <div className="flex items-center gap-2 shrink-0">
                                                        <div className="flex flex-col gap-0.5">
                                                            <button
                                                                onClick={(e) => { e.stopPropagation(); updateItemQuantity(item.name, item.unit, 1) }}
                                                                className="h-5 w-5 flex items-center justify-center rounded-sm bg-primary text-primary-foreground shadow-sm hover:bg-primary/90 transition-colors"
                                                            >
                                                                <Plus className="h-3 w-3" />
                                                            </button>
                                                            <button
                                                                onClick={(e) => { e.stopPropagation(); updateItemQuantity(item.name, item.unit, -1) }}
                                                                className="h-5 w-5 flex items-center justify-center rounded-sm bg-muted text-foreground border shadow-sm hover:bg-destructive hover:text-destructive-foreground transition-colors"
                                                            >
                                                        ...
                                                        <button
                                                            onClick={(e) => { e.stopPropagation(); removeItem(item.name, item.unit); }}
