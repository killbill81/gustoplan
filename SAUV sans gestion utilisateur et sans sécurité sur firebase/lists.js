...
export default function init() {
    ...
    async function fetchAllLists() {
        ...
        try {
            ...
            allLists = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            renderLists();
        } catch (error) {
    ...
    function addIngredientInput(ingredient = { quantity: '', name: '', unit: 'g' }) {
        ...
        unitSelect.className = 'ingredient-unit mt-1 block w-1/4 rounded-md border-gray-300 shadow-sm';
        const units = ['g', 'kg', 'ml', 'l', 'pièce(s)', 'c.à.s.', 'c.à.c.', 'pincée(s)'];
        units.forEach(unit => {
            const option = document.createElement('option');
            option.value = unit;
            option.textContent = unit;
            if (newIngredient.unit === unit) option.selected = true;
            unitSelect.appendChild(option);
        });
        unitSelect.addEventListener('change', (e) => { currentIngredients[index].unit = e.target.value; });
