...
export default function init() {
    ...
    async function loadPlanForWeek(week) {
        if (!db) return;
        const docRef = doc(db, "plans", `semaine-${week}`);
        try {
    ...
    async function saveCurrentPlan() {
        ...
        try {
            const docRef = doc(db, "plans", `semaine-${currentWeek}`);
    ...
    function exportToTxt() {
        ...
        shoppingList.forEach(item => {
            txtContent += `- ${item.totalQuantity} ${item.unit || ''} ${item.name}\n`;
        });
    ...
    function exportToPdf() {
        ...
        let y = 30;
        shoppingList.forEach(item => {
            doc.text(`- ${item.totalQuantity} ${item.unit || ''} ${item.name}`, 20, y);
            y += 10;
        });
    ...
    async function saveShoppingListToFirebase() {
        if (!db) return;
        try {
            const docRef = doc(db, "active_shopping_list", "current");
    ...
    function addIngredientToShoppingList(name, quantity, unit) {
        ...
        if (existingItem) {
            ...
            shoppingList.push({ name: name.trim(), totalQuantity: quantity, unit: unit, source: 'manual' });
        }
    ...
    function renderShoppingList() {
        ...
        shoppingList.forEach((item, index) => {
            ...
            quantitySpan.className = 'font-medium w-20 text-center';
        ...
        if (db) {
            try {
                ...
                plansSnapshot.forEach(doc => {
                    ...
                    const planMenuData = planData.menuData || {};
                    const planNumPeople = planData.numPeople || 1; // Use the people count from the specific plan

                    for (const slotId in planMenuData) {
                        ...
                        for (const mealInPlan of mealsInSlot) {
                            ...
                            mealToUse.ingredients.forEach(ingredient => {
                                ...
                                const finalQuantity = valuePerServing * planNumPeople; // Use the per-plan people count
                                ...
                                if (planIngredients.has(key)) {
                                    ...
                                    planIngredients.set(key, { name: name.trim(), totalQuantity: finalQuantity, unit: displayUnit, source: 'plan' });
                                }
                            });
                        }
                    }
                ...
                console.error("Error fetching all plans for shopping list generation:", error);
            }
        }
