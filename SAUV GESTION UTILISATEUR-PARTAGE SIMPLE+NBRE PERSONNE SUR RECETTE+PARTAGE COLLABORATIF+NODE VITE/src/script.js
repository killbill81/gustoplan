...
export default function init() {
    ...
    const elements = {
        ...
        showSharedPlansBtn: document.getElementById('show-shared-plans-btn'),
        sharedPlansModal: document.getElementById('shared-plans-modal'),
        closeSharedPlansModalBtn: document.getElementById('close-shared-plans-modal'),
        sharedPlansModalTitle: document.getElementById('shared-plans-modal-title'),
        sharedPlansModalContainer: document.getElementById('shared-plans-modal-container'),
    };
    ...
    let startDay = 'Lundi';
    let menuData = {};
    let servingsData = {}; // Stocke le nombre de personnes par repas (ex: {'0-lunch': 4})
    let remarksData = {};
    const shoppingList = [];
    ...
    async function loadPlanForWeek(week) {
        ...
        availablePlans = [];
        const ownPlanRef = doc(db, "plans", `${userId}_semaine-${week}`);

        try {
            const ownPlanSnap = await getDoc(ownPlanRef);
            if (ownPlanSnap.exists()) {
                // Document exists and we had permission to read it.
                availablePlans.push({ id: ownPlanSnap.id, name: "Mon plan", ...ownPlanSnap.data() });
            } else {
                // Document does not exist, create a fresh local plan.
                availablePlans.push({ id: `${userId}_semaine-${week}`, name: "Mon plan", menuData: {}, servingsData: {}, remarksData: {}, numPeople: 1, startDay: 'Lundi', userId: userId });
            }
        } catch (error) {
            console.error("Erreur de chargement du plan:", error);
            // This block will catch permission errors.
            // If we can't read the plan, we assume it's either invalid or doesn't exist.
            // We'll create a new empty local plan to allow the user to continue.
            console.log("Impossible de lire le plan existant (règles de sécurité ?). Création d'un nouveau plan local.");
            availablePlans.push({ id: `${userId}_semaine-${week}`, name: "Mon plan", menuData: {}, servingsData: {}, remarksData: {}, numPeople: 1, startDay: 'Lundi', userId: userId });
        }

        // The plan selector should only show the user's own plan(s)
        populatePlanSelector();
        // Load the user's own plan by default
    ...
    function loadPlan(plan) {
        ...
        // Set global state for the currently active, editable plan
        menuData = plan.menuData || {};
        servingsData = plan.servingsData || {};
        remarksData = plan.remarksData || {};
        defaultNumPeople = plan.defaultNumPeople || 1;
        ...
        // Render the default servings control
        if (elements.defaultServingsControl) {
            ...
            const defaultServingsComponent = createServingsControl(defaultNumPeople, async (newValue) => {
                ...
                // Re-render the main planner with the new default
                renderPlanner(elements.mealPlanGrid, { menuData, servingsData, remarksData, defaultNumPeople, startDay }, false);
        ...
        // The main plan is never shared, so buttons are always enabled.
        elements.sharePlanBtn.disabled = false;
        elements.clearMenuBtn.disabled = false;

        updateWeekDisplay();
        // Call the new generic render function for the main grid
        renderPlanner(elements.mealPlanGrid, plan, false);
    ...
    // ... (keep fetchSavedLists, etc. as they are) ...
    ...
    async function generateShoppingListFromPlan() {
        ...
        for (const slotId in menuData) {
            ...
            const numPeopleForMeal = parseInt(servingsData[servingsKey] || defaultNumPeople, 10);
        ...
        manualItemsMap.forEach((manualItem, key) => {
            if (planIngredients.has(key)) {
                ...
                // Potentially mark this as a mixed source if needed, for now, we prioritize plan source if it exists
            } else {
    ...
    function setupEventListeners() {
        ...
        elements.planSelect?.addEventListener('change', (event) => {
            ...
            loadPlan(availablePlans[selectedPlanIndex]);
        });
        ...
        elements.defaultServingsInput?.addEventListener('change', async (event) => {
            defaultNumPeople = parseInt(event.target.value) || 1;
            renderPlanner(elements.mealPlanGrid, { menuData, servingsData, remarksData, defaultNumPeople, startDay }, false); // Re-render to update placeholders in servings inputs
            await saveCurrentPlan();
            await generateShoppingListFromPlan();
        });
        ...
        elements.sharePlanBtn?.addEventListener('click', () => {
            const planData = { menuData, remarksData, defaultNumPeople, startDay, week: currentWeek, servingsData };
            openShareModal({ plan: planData });
        });

        elements.showSharedPlansBtn?.addEventListener('click', openSharedPlansModal);
        elements.closeSharedPlansModalBtn?.addEventListener('click', closeSharedPlansModal);
        elements.sharedPlansModal?.addEventListener('click', (e) => { 
            if (e.target === elements.sharedPlansModal) closeSharedPlansModal(); 
        });
    }
