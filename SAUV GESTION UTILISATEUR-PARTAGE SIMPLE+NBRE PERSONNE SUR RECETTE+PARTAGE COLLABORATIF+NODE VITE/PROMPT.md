...
Backend / Firestore : collection plans/{planId} avec champs minimaux :
...
Étape 2 — Friends list \& contrôle d’accès (pré-requis invitations)



Objectif : s’assurer que la logique d’amis est en place (uniquement amis acceptés peuvent être invités).

Tâches :



Vérifier / implémenter users/{uid}/friends/{friendUid} ou users/{uid}.friends avec statut accepted.



UI : page “Amis” affichant amis acceptés.

Critères :



Liste d’amis validée et accessible côté client.



Les appels d’invitation ne seront possibles que vers ces UIDs.



Étape 3 — Partage collaboratif : inviter via notifications (mode lecture+acceptation)
...
plans/{planId} : ajouter type: "collaborative", collaborators: { <uid>: "editor" }, owner.



notifications/{uid}/inbox/{notifId} : { type: "invitation", planId, fromUid, status:"pending", createdAt }
...
Acceptation : sur notifications l’utilisateur clique Accepter → on ajoute plans/{planId}.collaborators\[uid]= "editor" et copie lien vers userPlans/{uid} (si tu utilises index).
...
Étape 4 — Affichage visuel : distinguer plans et avatars ronds (UI)
...
Étape 5 — Partage de la liste de courses liée (lecture synchronisée, sans real-time)
...
Schéma : lists/{listId} lié par plans/{planId}.listId. Document items\[] : chaque item { name, qty, unit, fromRecipe: planRecipeId|null, addedByUid, manual: true|false, colorFlag, meal: "midi"/"soir", date }.



Lors du partage, s’assurer que plans/{planId}.listId est lisible par collaborators.



UI : zone Chercher ou ajouter + bouton Importer une liste (importer fichier JSON/CSV ou dupliquer une autre liste).



Conserver notifications par ingrédient : stocker metadata (plat concerné, date, midi/soir).
...
Étape 6 — Édition en écriture et synchronisation simple (polling court)
...
Côté client : mettre en place un listener via polling (fetch every 2–5s) pour plans/{planId} et lists/{listId}.



Actions : chaque modification écrit en Firestore avec champ updatedAt.
...
Étape 7 — Temps réel complet (Firestore real-time listeners ou WebSocket)
...
Étape 8 — Présence \& actions en cours (avatars + “Sophie ajoute …”)
...
Étape 9 — Gestion des conflits + historique (rollback)
...
Étape 10 — Gestion des accès avancée (owner retire un collab / quitter)
